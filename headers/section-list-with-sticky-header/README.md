...
In this one we are going to demonstrate how we can use a section list to implement the scrollable area with a sticky header.

First in our `_layout.tsx` file we are going to have the following for the `index` route.
...
In the `index.tsx` we are going to have the following:
